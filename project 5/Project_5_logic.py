...
#####################
    def valid_slots(self)->bool:
        ...
        for item in interesting:
            if self.other_turn() not in self.surrounding_chips(item):
                return True
    ...
    def valid_empty(self) -> bool:
        '''Checks if there is an available move if there are empty chips'''
        empty_chips = []
        for i in range(self._rows):
            for j in range(self._cols):
                ...
                if self._board[i][j] == '.':
                    ...
                    xD.append(i+1)
                    xD.append(j+1)
                    empty_chips.append(xD)
        #print(empty_chips)
        surrounding_of_empty = []
        for chip in empty_chips:
            surrounding_of_empty.extend(self.surrounding_chips(chip))
        #print(surrounding_of_empty)
        if self.other_turn() in surrounding_of_empty:
            return True
        else:
            return False
...
if __name__ == '__main__':
    c =GameState()
    c.new_game(4,4,'B')
    c._board =[['.','.','.','.'],['W','W','W','W'],['W','W','W','W'],['W','W','W','W']]
    c.print_board()
    c._turn = 'W'
    x = ['4', '4']
    d = ['4', '3']
